...
parser = argparse.ArgumentParser(
    description="Artwork Search Recommendation System"
    )
parser.add_argument("--tags_file", help="Path to the artist tags CSV file")
parser.add_argument(
    "--search_file", help="Path to the user search keywords TSV file"
    )
...
def main(config):
    ...
    artists_tags = csv_loader.load_data(
        tags_file, header=None, names=[config["tags_column"]]
        )
    search_keywords = csv_loader.load_data(search_file)

    stop_words = get_stop_words("english")
    search_processed = preprocess_input(
        search_keywords, keywords_column, token=config["tokens"],
        stop_words=stop_words
        )
    tags_processed = preprocess_input(
        artists_tags, tags_column, token=config["tokens"]
        )

    training_data = prepare_train(
        tags_processed[tags_column], search_processed[keywords_column]
        )
